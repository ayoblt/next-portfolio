  ...
  return (
    <div className="md:hidden w-fit h-fit">
      <m.ul
        ...
        initial={{ x: "-50%", display: "none" }}
        className={`${
          isOpen ? "flex" : "hidden"
        } fixed md:hidden px-4 py-7 flex-col justify-center items-center gap-3 bg-gray-700/70 bottom-24 w-5/6 left-1/2 -translate-x-1/2 rounded-2xl origin-bottom z-50`}
      >
        {NavRoutes.map((route) => (
          <m.li
            variants={{
